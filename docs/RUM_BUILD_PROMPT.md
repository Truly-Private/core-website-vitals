# RUM System — Build Prompt for core-website-vitals.replit.app

This file is a self-contained, paste-ready prompt to enhance the live app at
**https://core-website-vitals.replit.app/** by implementing the Real User
Monitoring (RUM) system. It contains both the build instructions and the full
specification inlined (originally `docs/RUM_SYSTEM.md`), so no other file needs
to be opened to act on it.
...
```
You are enhancing a live web app: https://core-website-vitals.replit.app/
Repo: https://github.com/Truly-Private/core-website-vitals
Branch with the design spec: claude/rum-system-implementation-68k7o3
Full specification to implement: docs/RUM_SYSTEM.md (also inlined below — it is
the source of truth).

GOAL
Extend the existing Core Web Vitals / SEO analyzer from a point-in-time LAB
auditor into a continuous FIELD-measurement product by adding a privacy-first
Real User Monitoring (RUM) system. Users should get their own Core Web Vitals
field data instantly instead of waiting on Google's 28-day CrUX average.

BUILD THE FOLLOWING

1) First-party RUM snippet (< 5KB gzipped, open source, async, zero render-blocking)
   - Collects all three Core Web Vitals in real time, per session:
     - LCP (with LCP element by coarse tag+role only — never id/text)
     - CLS (with coarse shift-source attribution)
     - INP with the THREE-PART breakdown: input delay / processing time /
       presentation delay (this breakdown is the headline diagnostic)
   - Context: device class, effective connection type, viewport, country-only.
   - Behavioral signals: click positions as viewport percentages (x%, y%),
     scroll depth, rage clicks (3+ rapid clicks), dead clicks (click with no
     DOM/nav/network effect), throttled mouse-movement sampling, session flow
     (ordered page path only).
   - Sends via sendBeacon/fetch over HTTPS only; flush on visibilitychange.

2) Privacy pipeline (PII-free by construction)
   - No cookies, no device identifiers, no fingerprinting, no cross-site tracking.
   - IP used ONLY at the edge for a country-only geo lookup (which also derives
     the privacy bucket), then DISCARDED before write. IP never reaches storage,
     logs, or rollups.
   - Never read element IDs, page text content, or form values.
   - Respect navigator.globalPrivacyControl and Do Not Track — collect/send
     nothing when set.
   - Cookie-free → no consent banner required in most jurisdictions.

3) Database: Neon (serverless Postgres) with isolation + residency + encryption
   - Database-per-tenant isolation: each customer account gets its OWN Neon
     database/project. NO shared tenant_id column — the database IS the boundary.
   - Regional placement: provision the tenant DB (or a regional shard) in the
     region matching its data-residency obligations (EU data in EU region, etc.).
   - Visitor regional privacy bucketing: each event carries privacy_bucket
     (GDPR | CCPA | APAC | OTHER) derived from the country-only edge lookup.
     The bucket drives (a) which regional DB the event lands in, (b) retention,
     (c) regime-specific processing. Honor PIPL/China cross-border rules: keep
     in-region or drop rather than transfer.
   - Encryption: TLS 1.3 in transit (snippet→edge→Neon); AES-256 at rest via
     Neon; PLUS per-tenant end-to-end envelope encryption — a per-tenant KMS CMK
     wraps data keys, sensitive fields/JSONB (behavior, attribution) are
     encrypted in the app BEFORE write so the DB/operator sees only ciphertext.
     Keep coarse aggregate/index columns (percentiles, country, privacy_bucket,
     timestamps) plaintext so rollups and routing work. CMK destruction =
     crypto-erase deletion.
   - Tables (full DDL inlined below):
     - rum_events: per-pageview raw — site_id, page_path, occurred_at,
       privacy_bucket, lcp_ms, cls, inp_ms, inp_input_delay_ms,
       inp_processing_ms, inp_presentation_ms, lcp_element, cls_sources(JSONB),
       device_class, connection_type, country, viewport, behavior(JSONB).
       Indexes on (site_id, occurred_at), (site_id, page_path), GIN(behavior).
     - rum_rollups: hourly/daily/weekly aggregations — p50/p75/p95 per metric,
       INP-breakdown p75s, heatmap_grid(JSONB), behavior_summary(JSONB),
       privacy_bucket, sample_count. Unique (site_id, page_path, bucket,
       bucket_start, privacy_bucket).

4) Dashboard — two new tabs on the site detail page
   - "Real Users": live CWV gauges (p75 + thresholds), INP breakdown viz, LCP
     element tracking, CLS source tracking, device/connection breakdown, per-page
     performance table, RUM-vs-CrUX comparison badges.
   - "Heatmaps": click/scroll/movement heatmaps rendered on a <canvas> overlaid
     on SERVER-captured page screenshots, rage-click hotspots, dead-click
     detection, scroll-depth visualization, session-flow diagrams.

5) New components
   <INPBreakdownBar>, <RUMvsCruxComparison>, <HeatmapOverlay> (canvas),
   <RageClickAlert>, <DeadClickAlert>, <ScrollDepthGauge>, <RUMInstallSnippet>,
   <SessionFlowDiagram>.

6) Pricing tiers (gate the features)
   - Free: no RUM (lab audits + CrUX only).
   - Pro: RUM + click/scroll heatmaps + rage/dead click detection;
     24h raw retention, 90-day rollups.
   - Agency: everything in Pro + movement heatmaps + session flow +
     page-level breakdowns; 7-day raw retention.

7) Update the customer-facing privacy copy to state: no PII, no cookies,
   country-only IP then discarded, no element IDs/text/form values, respects
   GPC/DNT, no consent banner in most jurisdictions, encrypted in transit
   (TLS 1.3) / at rest (AES-256) / per-tenant E2E, open-source snippet.

CONSTRAINTS
- Match the existing app's stack and conventions (the current app is a Python
  SEO analyzer with a Flask API — extend it; add the RUM ingest endpoint, the
  Neon-backed store, and the dashboard tabs consistent with what exists).
- Keep the snippet under 5KB gzipped; never let monitoring degrade performance.
- Implement the privacy guarantees as hard invariants, not config toggles.
- Provide DB migrations, the snippet source (publishable to GitHub), an ingest
  endpoint, and a rollup/aggregation job.

Implement incrementally:
schema → ingest endpoint → snippet → rollups → dashboard tabs → pricing gates.
```

---

# Full specification (inlined from `docs/RUM_SYSTEM.md`)
